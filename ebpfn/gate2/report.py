...
def gate2_verdict(variance: dict, ablation: dict) -> dict:
    ...
    if not variance["passes"]:
        code = DEAD
        why = (
            f"Part A FAIL: only {variance['frac_outside']:.0%} of real tasks fall outside the "
        ...
        code = INCONCLUSIVE
        why = (
            f"Part A passed, but the prior ladder produces near-zero within-task coverage spread "
        ...
        why = (
            f"Part A passed and coverage varies, but only {ablation['n_tasks']} tasks are shared "
        ...
        code = LINK
        why = (
            f"Part B PASS: across-prior fixed-effects correlation {ablation['fe_corr']:+.3f} "
        ...
        code = NO_LINK
        why = (
            f"Part A passed (coverage discriminates), but Part B is a true null: the fixed-effects "
        ...
        code = INCONCLUSIVE_POWER
        why = (
            f"Part A passed and coverage varies, but the Part B CI [{ablation['ci_lo']:+.3f}, "
